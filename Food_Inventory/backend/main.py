...
def ensure_schema():
    ...
    migrate_ingredients_add_brand_name()
    ...
    with engine.begin() as conn:
        ...
        conn.exec_driver_sql(
            ...
            CREATE TABLE ingredients_new (
              ...
              brand_name VARCHAR NOT NULL DEFAULT 'Brand',
        ...
        conn.exec_driver_sql(
            ...
            INSERT INTO ingredients_new (
              ...
              shelf_life_days, brand_name, expiration_date, default_unit_cost, cost_unit, ingredient_code, shelf_life
            )
            SELECT
              id, name, category, barcode, unit, quantity_on_hand, reorder_level,
              shelf_life_days, COALESCE(NULLIF(trim(brand_name), ''), 'Brand'),
              expiration_date, default_unit_cost, cost_unit, ingredient_code, shelf_life
            FROM ingredients
...
def migrate_ingredients_add_brand_name():
    # Ensure ingredients has brand_name with a safe default.
    with engine.begin() as conn:
        inspector = inspect(conn)
        if "ingredients" not in set(inspector.get_table_names()):
            return
        ...
        if "brand_name" not in columns:
            conn.exec_driver_sql("ALTER TABLE ingredients ADD COLUMN brand_name VARCHAR NOT NULL DEFAULT 'Brand'")
        conn.exec_driver_sql(
            "UPDATE ingredients SET brand_name = 'Brand' WHERE brand_name IS NULL OR trim(brand_name) = '' OR lower(trim(brand_name)) = 'no-brand'"
        )
...
app = FastAPI(title="Kitchen Food Inventory API", version="1.4.1")
...
def ingredient_to_schema(ingredient: Ingredient, ref_date: date, db: Session) -> IngredientOut:
    ...
    return IngredientOut(
        ...
        brand_name=ingredient.brand_name or "Brand",
...
def repair_purchase_order_integrity(db: Session):
    # Repair legacy NULL/blank IDs and ensure arrivals default to "-".
    po_rows = db.execute(text("SELECT rowid, id FROM purchase_orders ORDER BY rowid")).fetchall()
    used_po_ids: set[int] = set()
    for _, raw_id in po_rows:
        try:
            if raw_id is not None and str(raw_id).strip() != "":
                used_po_ids.add(int(str(raw_id)))
        except (TypeError, ValueError):
            continue
    next_po_id = (max(used_po_ids) if used_po_ids else 0) + 1
    for rowid, raw_id in po_rows:
        if raw_id is not None and str(raw_id).strip() != "":
            continue
        while next_po_id in used_po_ids:
            next_po_id += 1
        db.execute(
            text("UPDATE purchase_orders SET id = :id WHERE rowid = :rowid"),
            {"id": f"{next_po_id:02d}", "rowid": rowid},
        )
        used_po_ids.add(next_po_id)
        next_po_id += 1
    ...
    db.execute(
        text(
            ...
            UPDATE purchase_orders
            SET arrival = '-'
            WHERE arrival IS NULL OR trim(CAST(arrival AS TEXT)) = ''
            """
        )
    )

    poi_rows = db.execute(text("SELECT rowid, id FROM purchase_order_items ORDER BY rowid")).fetchall()
    used_poi_ids: set[int] = set()
    for _, raw_id in poi_rows:
        try:
            if raw_id is not None and str(raw_id).strip() != "":
                used_poi_ids.add(int(str(raw_id)))
        except (TypeError, ValueError):
            continue
    next_poi_id = (max(used_poi_ids) if used_poi_ids else 0) + 1
    for rowid, raw_id in poi_rows:
        if raw_id is not None and str(raw_id).strip() != "":
            continue
        while next_poi_id in used_poi_ids:
            next_poi_id += 1
        db.execute(
            text("UPDATE purchase_order_items SET id = :id WHERE rowid = :rowid"),
            {"id": f"{next_poi_id:02d}", "rowid": rowid},
        )
        used_poi_ids.add(next_poi_id)
        next_poi_id += 1

    # Backfill missing PO links on item rows using same ID where possible.
    first_po_id_row = db.execute(text("SELECT id FROM purchase_orders ORDER BY CAST(id AS INTEGER) ASC LIMIT 1")).fetchone()
    fallback_po_id = first_po_id_row[0] if first_po_id_row else None
    bad_link_rows = db.execute(
        text(
            """
            SELECT rowid, id
            FROM purchase_order_items
            WHERE purchase_order_id IS NULL OR trim(CAST(purchase_order_id AS TEXT)) = ''
            ORDER BY rowid
            """
        )
    ).fetchall()
    for rowid, item_id in bad_link_rows:
        candidate = None
        if item_id is not None and str(item_id).strip() != "":
            item_candidate = f"{int(str(item_id)):02d}" if str(item_id).isdigit() else str(item_id).strip()
            exists = db.execute(
                text("SELECT 1 FROM purchase_orders WHERE id = :id LIMIT 1"),
                {"id": item_candidate},
            ).fetchone()
            if exists:
                candidate = item_candidate
        if candidate is None:
            candidate = fallback_po_id
        if candidate is not None:
            db.execute(
                text("UPDATE purchase_order_items SET purchase_order_id = :purchase_order_id WHERE rowid = :rowid"),
                {"purchase_order_id": candidate, "rowid": rowid},
            )
...
def ensure_brands(db: Session):
    # Seed a fixed brand master list with 2-digit IDs.
    if db.query(Brand).count() > 0:
        return
    for idx, brand_name in enumerate(DEFAULT_BRANDS, start=1):
        db.add(Brand(id=f"{idx:02d}", brand_name=brand_name))
    db.flush()
...
def add_dataset_option(
    ...
    if dataset_key == "brands":
        exists = db.query(Brand).filter(Brand.brand_name == label).first()
        if exists:
            return DatasetOptionOut(
                id=int(exists.id),
                dataset_key=dataset_key,
                label=exists.brand_name,
                value=exists.brand_name,
            )
        row = Brand(id=next_lookup_id(db, Brand), brand_name=label)
        db.add(row)
        db.commit()
        db.refresh(row)
        return DatasetOptionOut(
...
def create_ingredient(
    ...
    if existing_by_name:
        ...
        payload_data["brand_name"] = (payload_data.get("brand_name") or "Brand").strip() or "Brand"
    ...
    db.execute(
        text(
            ...
            INSERT INTO ingredients (
                id, name, category, barcode, unit, quantity_on_hand, reorder_level,
                shelf_life_days, brand_name, expiration_date, default_unit_cost, cost_unit
            ) VALUES (
                :id, :name, :category, :barcode, :unit, :quantity_on_hand, :reorder_level,
                :shelf_life_days, :brand_name, :expiration_date, :default_unit_cost, :cost_unit
            )
        ...
        ),
        {
            ...
            "brand_name": payload_data["brand_name"],
...
def read_purchase_orders(db: Session = Depends(get_db), _: str = Depends(authorize("Rep"))):
    ...
    repair_purchase_order_integrity(db)
    rows = (
...
def create_purchase_order(
    ...
    repair_purchase_order_integrity(db)
    # Resolve ingredient IDs against actual DB values (accept both 1 and "01" input forms).
    try:
        requested_numeric_ids = [int(line.ingredient_id) for line in payload.items]
    except (TypeError, ValueError):
        raise HTTPException(status_code=400, detail="Invalid ingredient ID format")

    ingredient_id_rows = db.query(Ingredient.id).all()
    ingredient_id_map: dict[int, str] = {}
    for (raw_id,) in ingredient_id_rows:
        try:
            numeric = int(str(raw_id))
        except (TypeError, ValueError):
            continue
        raw_text = str(raw_id)
        existing = ingredient_id_map.get(numeric)
        # Prefer canonical 2-digit form when available.
        if existing is None or (len(raw_text) == 2 and raw_text.isdigit()):
            ingredient_id_map[numeric] = raw_text

    resolved_ingredient_ids: list[str] = []
    invalid_ids: list[int] = []
    for numeric_id in requested_numeric_ids:
        resolved = ingredient_id_map.get(numeric_id)
        if resolved is None:
            invalid_ids.append(numeric_id)
        else:
            resolved_ingredient_ids.append(resolved)
    if invalid_ids:
        invalid_text = ", ".join(str(value) for value in sorted(set(invalid_ids)))
        raise HTTPException(status_code=400, detail=f"One or more ingredient IDs are invalid: {invalid_text}")
    ...
    # Create PO header/lines with explicit IDs for TEXT PK schemas.
    po_id = next_lookup_id(db, PurchaseOrder)
    db.execute(
        text(
            """
            INSERT INTO purchase_orders (id, supplier, po_status, created_at, arrival)
            VALUES (:id, :supplier, :po_status, :created_at, :arrival)
            """
        ),
        {
            "id": po_id,
            "supplier": payload.supplier,
            "po_status": status_row.id,
    ...
    for index, line in enumerate(payload.items):
        line_id = next_lookup_id(db, PurchaseOrderItem)
        db.execute(
            text(
                """
                INSERT INTO purchase_order_items (
                    id, purchase_order_id, ingredient_id, quantity_ordered, unit_cost, cost_unit
                ) VALUES (
                    :id, :purchase_order_id, :ingredient_id, :quantity_ordered, :unit_cost, :cost_unit
                )
                """
            ),
            {
                ...
                "ingredient_id": resolved_ingredient_ids[index],
    ...
    if not po:
        raise HTTPException(status_code=500, detail="Purchase order saved but could not be reloaded")
