...
# v1.8 mode contracts. The current implementation still maps onto the legacy
# render pipeline, but these definitions provide a stable target for the
# mode-by-mode refactor while preserving the existing UI/options.
MODE_SPECS = [
    ("image", "Image Pages Only"),
    ("readable_image", "Readable Image Pages"),
    ("hybrid", "Hybrid (Page Image + Text)"),
    ("overlay", "Flowable Overlay (Text on Page Images)"),
    ("text", "Text Only"),
]
...
def render_page_image(
    ...
    if clip_to_content:
        try:
            probe_zoom = max(0.2, min(0.5, 220.0 / max(1.0, rect.width)))
            probe = page.get_pixmap(
                matrix=fitz.Matrix(probe_zoom, probe_zoom),
                alpha=False,
            ...
            if pw > 0 and ph > 0:
                ...
                max_x, max_y = -1, -1
                for y in range(ph):
                    row_off = y * probe.stride
                    for x in range(pw):
                        if buf[row_off + x] < threshold:
                            if x < min_x:
                                min_x = x
                            if y < min_y:
                                min_y = y
                ...
                if max_x >= 0 and max_y >= 0:
                    scale_x = rect.width / pw
                    scale_y = rect.height / ph
                    raster_rect = fitz.Rect(
                        rect.x0 + min_x * scale_x,
                        rect.y0 + min_y * scale_y,
                        rect.x0 + (max_x + 1) * scale_x,
                        rect.y0 + (max_y + 1) * scale_y,
                    )
                    pad_x = max(4.0, raster_rect.width * 0.008)
                    pad_y = max(8.0, raster_rect.height * 0.015)
                    clip_rect = fitz.Rect(
                        max(rect.x0, raster_rect.x0 - pad_x),
                        max(rect.y0, raster_rect.y0 - pad_y),
                        min(rect.x1, raster_rect.x1 + pad_x),
                        min(rect.y1, raster_rect.y1 + pad_y),
                    )
        except Exception:
            clip_rect = rect

        content_boxes: List[fitz.Rect] = []
        try:
            for block in page.get_text("blocks"):
                if len(block) >= 4:
                    x0, y0, x1, y1 = map(float, block[:4])
                    if x1 > x0 and y1 > y0:
                        content_boxes.append(fitz.Rect(x0, y0, x1, y1))
        except Exception:
            pass
        try:
            for drawing in page.get_drawings():
                rect_d = drawing.get("rect")
                if rect_d is not None:
                    content_boxes.append(fitz.Rect(rect_d))
        except Exception:
            pass

        if content_boxes:
            union = fitz.Rect(content_boxes[0])
            for box in content_boxes[1:]:
                union |= box
            pad_x = max(5.0, union.width * 0.01)
            pad_y = max(10.0, union.height * 0.02)
            bounds_rect = fitz.Rect(
                max(rect.x0, union.x0 - pad_x),
                max(rect.y0, union.y0 - pad_y),
                min(rect.x1, union.x1 + pad_x),
                min(rect.y1, union.y1 + pad_y),
            )
            clip_rect &= bounds_rect
            if clip_rect.width < 100 or clip_rect.height < 100:
                clip_rect = rect

    render_scale = 1.0
    if clip_to_content and not preserve_pdf_dimensions:
        # Slight oversampling keeps small textbook text crisper after scaling in readers.
        render_scale = 1.22

    if preserve_pdf_dimensions:
        zoom = 1.0
    else:
        zoom = (max_width / clip_rect.width) * render_scale if clip_rect.width > 0 else 1.0
    mat = fitz.Matrix(zoom, zoom)
    pix = page.get_pixmap(matrix=mat, clip=clip_rect, alpha=False)

    img = Image.frombytes("RGB", [pix.width, pix.height], pix.samples)
    if clip_to_content and not preserve_pdf_dimensions:
        target_w = max(1, int(round(max_width)))
        target_h = max(1, int(round((img.height / max(1, img.width)) * target_w)))
        if img.width > target_w:
            img = img.resize((target_w, target_h), Image.Resampling.LANCZOS)
        img = img.filter(ImageFilter.UnsharpMask(radius=0.8, percent=135, threshold=2))
...
def next_available_output_path(out_dir: Path, file_name: str) -> Path:
    ...
    for n in range(2, 1000):
        numbered = out_dir / f"{stem} {n:02d}{suffix}"
        if not numbered.exists():
            return numbered
    return out_dir / f"{stem} {dt.datetime.now():%Y%m%d_%H%M%S}{suffix}"
...
def apply_output_budget(
    ...
    width = int(image_max_width)
    quality = int(image_quality)
    fmt = one_line(image_format_mode).lower() or "hybrid"

    if page_count < 250:
        return width, quality, fmt

    # Keep large books under roughly 100 MB / 1000 pages by progressively
    # reducing render width and preferring JPEG for page-image-heavy modes.
    if page_count >= 1000:
        width = min(width, 1050)
        quality = min(quality, 74)
    elif page_count >= 750:
        width = min(width, 1125)
        quality = min(quality, 76)
    elif page_count >= 500:
        width = min(width, 1200)
        quality = min(quality, 78)
    else:
        width = min(width, 1325)
        quality = min(quality, 82)

    if render_mode in {"image", "readable_image"}:
        fmt = "auto_jpeg"
    elif fmt == "png" and page_count >= 750:
        fmt = "hybrid"

    return max(700, width), max(45, quality), fmt
...
def convert_pdf_to_epub(
    ...
    css = epub.EpubItem(
        ...
        content=(
            ...
            ".folio{color:#666;font-size:.9em;text-align:center;margin:.55em 0 0 0;}"
            ...
            "body.image-only .folio{margin:.35em 0 .2em 0;}"
    ...
    if cover_bytes is None:
        ...
        cover_bytes, _, _, cover_fmt = render_page_image(
            ...
            clip_to_content=(render_mode == "readable_image"),
        )
    ...
    if render_mode not in {"hybrid", "image", "readable_image", "text", "overlay"}:
        render_mode = "hybrid"
    image_max_width, image_quality, image_format_mode = apply_output_budget(
        doc.page_count,
        render_mode,
        image_max_width,
        image_quality,
    ...
    for i in range(doc.page_count):
        ...
        if render_mode in {"image", "readable_image"}:
            chap.content = (
                ...
                f"<div class='folio'>{html_escape(str(shown_page_label))}</div>"
                "</div></body></html>"
            )
    ...
    if not kept_pdf_pages and doc.page_count > 0:
        ...
        shown_page = logical_page_number(page_no, logical_start_page)
        shown_page_label = str(shown_page) if shown_page >= 1 else str(page_no)
        raw_text = page.get_text("text") if render_mode not in {"image", "readable_image"} else ""
        force_image_only = render_mode in {"hybrid", "text"} and (
            page_looks_toc_like(raw_text) or page_requires_image_only(page, raw_text)
        )
        use_image = render_mode != "text" or page_needs_rare_image(page, raw_text) or force_image_only
    ...
    out_path = next_available_output_path(out_dir, out_name)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.mode = QComboBox()
        self.mode.addItems([label for _mode_key, label in MODE_SPECS])
    ...
    def selected_mode_key(self) -> str:
        idx = max(0, min(self.mode.currentIndex(), len(MODE_SPECS) - 1))
        return MODE_SPECS[idx][0]

    def on_mode_changed(self):
        image_only = self.selected_mode_key() in {"image", "readable_image"}
        if image_only:
            self.image_format.setCurrentText("PNG (Lossless)")
            self.image_format.setEnabled(False)
            mode_key = self.selected_mode_key()
            note = (
                "Readable-image mode: page format forced to PNG (Lossless) and page margins are cropped."
                if mode_key == "readable_image"
                else "Image-only mode: page format forced to PNG (Lossless)."
            )
            self.log_msg(note)
        else:
    ...
    def convert(self):
        ...
        if mode in {"image", "readable_image"}:
            image_format_mode = "png"
        cover_source = "auto"
