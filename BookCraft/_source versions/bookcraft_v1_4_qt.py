...
class ConvertWorker(QThread):
    ...
    def run(self):
        ...
        if self.cover_image:
            cmd.extend(["--cover-image", self.cover_image])
        try:
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.script_path = Path(__file__).resolve().parent / "bookcraft_v1_4.py"
        ...
        self.top_progress.setRange(0, 1)
        ...
        lgrid.addWidget(QLabel("Image max width"), 9, 0)
        lgrid.addWidget(self.width, 9, 1)
        lgrid.addWidget(QLabel("JPEG quality (25-85)"), 10, 0)
        lgrid.addWidget(self.quality, 10, 1)
        lgrid.addWidget(self.auto_open, 11, 0, 1, 2)

        btnrow = QHBoxLayout()
        btnrow.addWidget(self.btn_create)
        btnrow.addWidget(self.btn_clear)
        btnrow.addWidget(self.btn_quit)
        lgrid.addLayout(btnrow, 12, 0, 1, 2)
    ...
    def set_busy(self, busy: bool):
        for w in [self.btn_pdf, self.btn_out, self.btn_cover, self.mode, self.preset, self.cover_source, self.width, self.quality, self.btn_create]:
    ...
    def convert(self):
        ...
        self.log_msg(f"Starting conversion mode={mode}, cover={cover_source}, width={width}, quality={quality}")
        ...
        if cover_image and not os.path.exists(cover_image):
            ...
            self.set_busy(False)
            return
        self.worker = ConvertWorker(self.script_path, pdf, out, mode, width, quality, cover_source, cover_image)
